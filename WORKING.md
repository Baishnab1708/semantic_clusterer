# How semantic-clusterer Works

This document explains the **internals** of `semantic-clusterer`: the
...
If you just want to use the library, start with [`README.md`](README.md) and
...
3. [Preprocessing and deduplication](#3-preprocessing-and-deduplication)
4. [Embedding layer](#4-embedding-layer)
5. [Dataset profiling](#5-dataset-profiling)
6. [Dimension bands](#6-dimension-bands)
7. [Tier routing](#7-tier-routing)
8. [SemanticClusterer tiers](#8-semanticclusterer-tiers)
9. [The scoring objective](#9-the-scoring-objective)
10. [Granularity system](#10-granularity-system)
11. [Post-processing](#11-post-processing)
12. [SemanticKSplit internals](#12-semanticksplit-internals)
13. [Keyword and topic-label generation](#13-keyword-and-topic-label-generation)
14. [The fitted state, persistence, and prediction](#14-the-fitted-state-persistence-and-prediction)
15. [Out-of-distribution calibration](#15-out-of-distribution-calibration)
16. [Determinism model](#16-determinism-model)
17. [Quality floors and confidence](#17-quality-floors-and-confidence)
18. [Oversized datasets](#18-oversized-datasets)
19. [Module map](#19-module-map)
...
```
        texts: List[str]
              ...
              ▼
   ┌──────────────────────┐
   │ 1. Preprocess         │  clean → dedupe → index map
   └──────────────────────┘
              │  unique texts
              ▼
   ┌──────────────────────┐
   │ 2. Embed              │  via built-in ONNX or custom model
   └──────────────────────┘
              │  (N, D) float32
              ▼
   ┌──────────────────────┐
   │ 3. Resolve dim band   │  D → low / mid / high / xhigh
   └──────────────────────┘
              │
              ▼
   ┌──────────────────────┐
   │ 4. Profile dataset    │  rank, density, duplicates, tendency, ...
   └──────────────────────┘
              │
              ▼
   ┌──────────────────────┐
   │ 5. Route to tier      │  N → tiny / small / medium / large
   │    (or k-algorithm)   │  (KSplit: (tier, k) → algorithm)
   └──────────────────────┘
              │
              ▼
   ┌──────────────────────┐
   │ 6. Cluster            │  tier-specific pipeline
   └──────────────────────┘
              │  labels on unique rows
              ▼
   ┌──────────────────────┐
   │ 7. Post-process       │  noise recovery, splitting, merging
   └──────────────────────┘
              │
              ▼
   ┌──────────────────────┐
   │ 8. Quality floor      │  warn + lower confidence if weak
   └──────────────────────┘
              │
              ▼
   ┌──────────────────────┐
   │ 9. Project labels     │  unique → original input order
   └──────────────────────┘
              │
              ▼
        labels: np.int32 (len(texts),)
```
...
- CPU inference via `onnxruntime` (no PyTorch). Threads set to `os.cpu_count()`.
...
> **⚠️ Large pipeline validation status:** The `large` tier has been upgraded to match the multi-reduction and granularity post-processing architecture of the medium tier, but **has not yet been benchmarked or tested** in v0.1.0 releases. Tiny, small, and medium tiers are fully benchmarked and verified.
...
- [`README.md`](README.md) — overview and API surface.

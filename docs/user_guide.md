# semantic_clusterer - User Guide

A practical, task-oriented guide to `semantic_clusterer`. It starts with the
...
The zero-config path. No model, no parameters, just text in and groups out.
...
> [!NOTE]
> For details on how dimension bands resolve your custom embedder's output size to select the appropriate UMAP and HDBSCAN search space, see [How It Works: Dimension bands](working.md#6-dimension-bands).
...
> [!NOTE]
> For details on how these knobs affect internal search space scoring and parameters, see [How It Works: Granularity system](working.md#10-granularity-system) and [How It Works: Quality profile](working.md#12-semanticksplit-internals).
...
tiny tier) ignore the restart count, since running them repeatedly gives the same
...
> [!NOTE]
> For details on the model manifest schema, folder structure, and prediction algorithms, see [How It Works: Fitted state persistence](working.md#14-the-fitted-state-persistence-and-prediction).
...
This is intentional, as it avoids arbitrary Python and locking the saved
...
> [!NOTE]
> For the mathematical formulas, threshold scaling factors, and boundary calculations, see [How It Works: Out-of-distribution calibration](working.md#15-out-of-distribution-calibration).
...
For observability, such as logging, dashboards, and debugging, use the `*_with_report`
methods. They return both the labels and a structured `ClusteringReport`.

> [!NOTE]
> For details on how the scoring metrics (coverage, cohesion, separation) are computed and weighted, see [How It Works: The scoring objective](working.md#9-the-scoring-objective).
...
| `library_version` | The `semantic_clusterer` version |
...
| `coverage` | 0..1 | Better, the fraction of points clustered (`1 - noise_ratio`) |
| `cohesion` | 0..1 | Better, the average within-cluster similarity |
| `separation` | 0..1 | Better, the average distance between cluster centroids |
| `stability` | 0..1 | Better, the size balance across clusters |
| `density` | 0..1 | Better, the HDBSCAN validity (when available) |
| `largest_ratio` | 0..1 | Lower, the size of the biggest cluster as a fraction of N |
| `fragmentation` | 0..1 | Lower, the fraction of micro-clusters |
| `noise_ratio` | 0..1 | Lower, the fraction of unclustered points |
| `n_clusters` | int | - |
...
|-------|------|---------|---------|
| `cluster_granularity` | `"fine"\|"balanced"\|"coarse"` | `"balanced"` | Primary knob for cluster count vs. merging |
...
| Field | Type | Default | Meaning |
|-------|------|---------|---------|
| `quality` | `"fast"\|"balanced"\|"best"` | `"balanced"` | Primary knob for restart count |
...
- The same `semantic_clusterer` version.
...
> [!WARNING]
> **Large pipeline status:** The `large` tier (N > 50 000) has been upgraded
...
- There is no replacement knob, and the behaviour you previously got from
...
- [`index.md`](index.md), quick overview and API surface.
